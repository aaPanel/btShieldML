...
func (r *HtmlReporter) Generate(results []*types.ScanResult, outputPath string) error {
	...
	// fileTypeLabelsJSON := strings.Join(fileTypeLabels, ",")
	// fileTypeValuesJSON := strings.Join(strings.Split(fmt.Sprint(fileTypeValues), " "), ",")
    ...
    <div class="container">
        ...
        <div class="summary">
            ...
            <ul>
                ...
                <li class="risk-count"><i class="fas fa-radiation"></i>木马文件数量 (Critical)：<span>` + fmt.Sprintf("%d", trojanFiles) + `</span></li>
            </ul>
        </div>
	...
	if len(problemFiles) > 0 {
		for i, res := range problemFiles {
			...
			if res.Error != nil {
				...
				case types.RiskHigh, types.RiskMedium, types.RiskLow:
					riskDesc = "疑似木马"
				...
				case types.RiskNone:
					...
					dataFilter = "normal"
					recommendation = "该文件未检测到异常，可以正常使用。"
				default:
					riskDesc = "未知"
					riskClass = "risk-unknown"
					riskIcon = "fas fa-question-circle"
			...
			`, dataFilter, int(res.OverallRisk), fileName, i, fileName, filePath, riskClass, riskIcon, riskDesc, i))
			...
			`, i, fileName, fileSize, modTime, fileMD5, filePath, riskClass, riskIcon, riskDesc, recommendation))
		}
	} else {
...
`)
	...
	// logging.InfoLogger.Printf("HTML report successfully generated: %s", outputPath)
	return nil
}
